...
def main():
    ...
    # Get current flow
    data = module.api.get(f"/admin/realms/{ realm }/authentication/flows")
    if id:
    ...
    if state == 'present' and current_flow is None:
        ...
        if not module.check_mode:
            module.api.post(f"/admin/realms/{ realm }/authentication/flows", payload=new_flow)
    ...
    if state == 'absent':
        if current_flow is not None:
            ...
            if not module.check_mode:
                module.api.delete(f"/admin/realms/{ realm }/authentication/flows/{current_flow['id']}")
    ...
    if current_flow != new_flow:
        ...
        if not module.check_mode:
            module.api.put(f"/admin/realms/{ realm }/authentication/flows/{new_flow['id']}", payload=new_flow)
    ...
    if bind is not None:
        ...
        if current_bind != bind:
            ...
            if not module.check_mode:
                module.api.put(f"/admin/realms/{ realm }/", payload={f"{bind}Flow": name})
    ...
    if state == 'present' and executions is not None:
        current_executions = module.api.get(f"/admin/realms/{ realm }/authentication/flows/{ name }/executions")
        ...
        while idx < len(executions):
            ...
            if current_execution and current_execution['level'] > execution['level']:
                result['changed'] = True
                if not module.check_mode:
                    module.api.delete(f"/admin/realms/{ realm }/authentication/executions/{new_executions[idx]['id']}")
                    new_executions = module.api.get(f"/admin/realms/{ realm }/authentication/flows/{ quote(name) }/executions")
                else:
            ...
            if 'authenticator' in execution and (
                ...
                if not module.check_mode:
                    ...
                    module.api.post(f"/admin/realms/{ realm }/authentication/flows/{ flow_name_url }/executions/execution", payload=payload)

                    new_executions = module.api.get(f"/admin/realms/{ realm }/authentication/flows/{ flow_name_url }/executions")
            ...
            if 'name' in execution and (
                ...
                if not module.check_mode:
                    ...
                    flow_name_url = quote(execution.get('flow_name', name))
                    module.api.post(f"/admin/realms/{ realm }/authentication/flows/{ flow_name_url  }/executions/flow", payload=payload)
            ...
            if not module.check_mode and current_execution['index'] > execution['index']:
                for i in range(execution['index'], current_execution['index']):
                    module.api.post(f"/admin/realms/{ realm }/authentication/executions/{current_execution['id']}/raise-priority", payload=None)
            ...
            if current_execution.get('providerId', None) == execution.get('authenticator', None):
                ...
                if current_execution != new_execution:
                    result['changed'] = True
                    if not module.check_mode:
                        module.api.put(f"/admin/realms/{ realm }/authentication/flows/{ name }/executions", payload=new_execution)
                    else:
                        new_executions[idx] = new_execution
            if not module.check_mode:
                new_executions = module.api.get(f"/admin/realms/{ realm }/authentication/flows/{ name }/executions")
        ...
        while len(new_executions) > len(executions):
            result['changed'] = True
            if not module.check_mode:
                module.api.delete(f"/admin/realms/{ realm }/authentication/executions/{new_executions[-1]['id']}")
